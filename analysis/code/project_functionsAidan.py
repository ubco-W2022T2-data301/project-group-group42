...
def process_temp_dataset():
    ...
    temp_df["year"] = [x.year for x in temp_df["dt"]]
    temp_df = temp_df.groupby([temp_df["year"],temp_df["Country"]])["AverageTemperature"].mean(numeric_only=True).reset_index()
    return temp_df

def pivoted_temp_dataset():
    temp_df = pd.read_csv("../data/raw/GlobalLandTemperaturesByCountry.csv")
    temp_df["dt"] = (temp_df["dt"].apply(pd.to_datetime)).dt.date #Converts to datetime (Takes a while)
    ...
    temp_df = temp_df[temp_df["Country"].isin(world["name"].unique().tolist())] #Gets rid of any country which is not in world data (basically small island nations)
    
    pivoted_df = temp_df.pivot(index="Country",columns=["year"],values=["AverageTemperature"]) #pivots so each country is a row and the temperature at each year is the columns
